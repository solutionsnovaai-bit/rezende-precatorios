...
import { ArrowDown, ArrowUpRight, Check, ChevronDown, ChevronLeft, ChevronRight, Copy, FileCheck2, Handshake, Mail, Menu, Pause, Play, ShieldCheck, X } from 'lucide-react'
import { chapters, COMPANY, faqs, processSteps } from './content'
import BrandIntro from './components/BrandIntro'
import { useSmoothScroll } from './hooks/useSmoothScroll'
import { aboutSizes, gallerySizes, imageSrcSet } from './assets'

const ease = [0.22, 1, 0.36, 1] as const

function Logo({ className = '' }: { className?: string }) {
  return <span className={`brand-logo ${className}`}><img src="/images/logo-original.webp" width="1254" height="1254" alt="Rezende — precatórios e investimentos" fetchPriority="high" /></span>
}
...
function Header({ open, setOpen }: { open: boolean; setOpen: (open: boolean) => void }) {
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Tab') {
        const elements = [menuButton.current, ...Array.from(menuPanel.current?.querySelectorAll<HTMLAnchorElement>('a') ?? [])].filter(Boolean) as HTMLElement[]
...
function Hero({ ready }: { ready: boolean }) {
  ...
  return <section id="inicio" className="hero" ref={heroRef} onPointerMove={move} onPointerLeave={() => { mouseX.set(0); mouseY.set(0) }}>
    ...
    <div className="hero-content">
      ...
      <h1>{['Seu tempo', 'vale mais que', 'a espera.'].map((line, i) => <span className="line-mask" key={line}><motion.span initial={reduced ? false : { y: '110%' }} animate={ready ? { y: 0 } : {}} transition={{ delay: 0.15 + i * 0.13, duration: 1.1, ease }} className={i === 2 ? 'serif-italic' : ''}>{line}</motion.span></span>)}</h1>
...
function Gallery() {
  ...
  const jump = (next: number) => setSlide([next, next > index ? 1 : -1])
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => { if (!document.hidden) go(1) }, 6500)
    return () => clearInterval(timer)
  }, [playing, hovered, visible, reduced, go, index])
  useEffect(() => {
  ...
  return <section className="gallery-section" aria-label="Possibilidades para o seu próximo capítulo" ref={galleryRef}>
    <div className="gallery-heading wrap"><Reveal><span className="eyebrow">O QUE VOCÊ FARIA COM ESSE NOVO TEMPO?</span><h2>O próximo capítulo<br />pode ser <em>extraordinário.</em></h2></Reveal><Reveal className="gallery-intro" delay={0.15}><p>Seu precatório é um direito.<br />O que ele pode tornar possível<br />é uma escolha sua.</p><span className="gallery-small-line" /></Reveal></div>
    <div className="gallery-stage" tabIndex={0} role="region" aria-roledescription="carrossel" aria-label="Inspirações; use as setas para navegar" onKeyDown={e => { if (e.key === 'ArrowRight') { e.preventDefault(); go(1) } if (e.key === 'ArrowLeft') { e.preventDefault(); go(-1) } }} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} onFocus={() => setHovered(true)} onBlur={e => { if (!e.currentTarget.contains(e.relatedTarget)) setHovered(false) }}>
      <div className="gallery-visual" onPointerDown={e => { start.current = { x: e.clientX, y: e.clientY }; e.currentTarget.setPointerCapture(e.pointerId) }} onPointerUp={e => { if (start.current) { const dx = e.clientX - start.current.x; const dy = e.clientY - start.current.y; if (Math.abs(dx) > 45 && Math.abs(dx) > Math.abs(dy)) go(dx < 0 ? 1 : -1) } start.current = null }} onPointerCancel={() => { start.current = null }}>
        <AnimatePresence initial={false} custom={direction}>
          <motion.img key={slide.image} custom={direction} variants={{ enter: (d: number) => ({ opacity: 0, scale: 1.075, x: reduced ? 0 : d * 45 }), center: { opacity: 1, scale: 1, x: 0 }, exit: (d: number) => ({ opacity: 0, x: reduced ? 0 : d * -45 }) }} initial="enter" animate="center" exit="exit" transition={{ duration: reduced ? 0.05 : 0.9, ease }} src={`/images/${slide.image}.webp`} srcSet={imageSrcSet(slide.image)} sizes={gallerySizes} decoding="async" width="1448" height="1086" alt={slide.alt} loading="lazy" draggable={false} />
        </AnimatePresence>
        <span className="image-label">A VIDA TEM OUTROS PLANOS PARA VOCÊ.</span>
      </div>
      <div className="gallery-story" aria-live={playing ? 'off' : 'polite'} aria-atomic="true">
        <span className="chapter-number" aria-hidden="true">{String(index + 1).padStart(2, '0')}</span>
        <AnimatePresence mode="wait"><motion.div key={index} initial={reduced ? false : { opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} transition={{ duration: 0.3 }}><span className="eyebrow">{slide.category}</span><h3>{slide.title}</h3><p>{slide.text}</p></motion.div></AnimatePresence>
    ...
    <div className="gallery-bottom wrap">
      <div className="gallery-pagination" aria-label="Selecionar imagem">{chapters.map((c, i) => <button key={c.image} aria-label={`Ver imagem ${i + 1}: ${c.category}`} aria-current={index === i ? 'true' : undefined} onClick={() => jump(i)}><span className={index === i ? 'active' : ''} /></button>)}</div>
      <div className="gallery-controls"><span className="gallery-counter"><strong>{String(index + 1).padStart(2, '0')}</strong> / {String(chapters.length).padStart(2, '0')}</span>{!reduced && <button className="icon-button auto-button" aria-label={playing ? 'Pausar carrossel' : 'Reproduzir carrossel automaticamente'} aria-pressed={playing} onClick={() => setPlaying(!playing)}>{playing ? <Pause size={15} /> : <Play size={15} />}</button>}<button className="icon-button" aria-label="Imagem anterior" onClick={() => go(-1)}><ChevronLeft size={21} /></button><button className="icon-button" aria-label="Próxima imagem" onClick={() => go(1)}><ChevronRight size={21} /></button></div>
    </div>
...
function Footer() {
  return <footer className="site-footer wrap"><div className="footer-main"><a href="#inicio" aria-label="Voltar ao início"><Logo /></a><p>Confiança para decidir.<br /><em>Liberdade para viver.</em></p><a href="#inicio" className="back-top">VOLTAR AO TOPO <ArrowUpRight size={19} /></a></div><div className="footer-bottom"><span>© 2026 Rezende Precatórios. Todos os direitos reservados.</span><span>CNPJ: {COMPANY.cnpj}</span><a href={`mailto:${COMPANY.email}`}>Fale com a Rezende <ArrowUpRight size={13} /></a></div><p className="footer-legal">A antecipação está sujeita à análise do crédito, da documentação e à formalização. Valores e prazos são definidos na proposta e no contrato.</p></footer>
}
